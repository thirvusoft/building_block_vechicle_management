...
function vehicle(frm,cdt,cdn){
	...
	if( row.frequency !=""){
            if(!row.from_date){}
            if(!license){
                frappe.msgprint("Please fill the <b>License Plate<b> Number")
                cur_frm.scroll_to_field('license_plate')
                }
            if(row.day_before==1){
                  let day=1
                    frappe.call({
                        method:"building_block_vechicle_management.custom.py.vehicle.todate",
                        args:{
                            'diff':row.frequency, 
                            'fdate':row.from_date,
                            'duaration':day ? day:'', 
